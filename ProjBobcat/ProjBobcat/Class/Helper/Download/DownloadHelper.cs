...
using TAIGO.ECore;
using TAIGO.ECore.Entities;
using TAIGO.ECore.MLibs.Class.Helper.Download;
...
public static class DownloadHelper
{
    ...
    private static ConcurrentDictionary<string, RetryCounter> Domains { get; set; } = [];
#region 下载数据
    ...
    public static async Task DownloadData(AbstractDownloadBase downloadFile, DownloadSettings downloadSettings)
    {
        ...
        for (var i = 0; i <= trials; i++)
        {
            ...
            downloadFile.RetryCount = i;
            var host = new Uri(downloadFile.GetDownloadUrl()).Host;
            var domain = Domains.GetOrAdd(host, _ => new RetryCounter());
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    downloadFile.OnChanged(
                        speed,
                        ProgressValue.Create(downloadedBytesCount, responseLength),
                ...
                if (downloadSettings.CheckFile && !string.IsNullOrEmpty(downloadFile.CheckSum))
                {
                    ...
                    var checkSum = Convert.ToHexString(await downloadSettings.HashDataAsync(outputStream, cts.Token));
            ...
            {
                if(domain.Retry < 3)
                    Interlocked.Increment(ref domain.Retry);
                else if (domain.Retry == 3)
                {
                    Logger.Info($"Domain {host} is blocked.");
                }
            }
            catch (Exception e)
            {
                await Task.Delay(250);
                exceptions.Add(new Exception($"FAIL {downloadFile.FileName}: {downloadFile.GetDownloadUrl()} {e.Message}"));
            }
        }

        if(exceptions.Count == 0)
            exceptions.Add(new Exception("All domains is timout"));
    ...
    /// <param name="df"></param>
    /// <param name="downloadSettings"></param>
    public static Task AdvancedDownloadFile(AbstractDownloadBase df, DownloadSettings downloadSettings)
    {
        ...
        DownloadSettings downloadSettings )
    {
        ...
        for (var r = 0; r <= trials; r++)
        {
            ...
            try
            {
                ...
                using var headReq = new HttpRequestMessage(HttpMethod.Head, downloadFile.GetDownloadUrl()  );
                ...
                using var rangeGetMessage = new HttpRequestMessage(HttpMethod.Get, downloadFile.GetDownloadUrl());
